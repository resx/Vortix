...
use base64::prelude::{BASE64_STANDARD as BASE64_STD, Engine as _};
...
pub const fn sync_kdf_iterations() -> u32 {
...
pub fn encrypt_sync_blob(plaintext: &[u8], key: &[u8; 32]) -> Result<(Vec<u8>, Vec<u8>), String> {
...
pub fn decrypt_sync_blob(ciphertext: &[u8], key: &[u8; 32], iv: &[u8]) -> Result<Vec<u8>, String> {
    if iv.len() != SYNC_IV_LENGTH {
...
pub fn encrypt_sync_field(plaintext: &str, key: &[u8; 32]) -> Result<String, String> {
    ...
    let mut ciphertext = cipher
        ...
        .map_err(|_| "encrypt failed".to_string())?;
    if ciphertext.len() < SYNC_TAG_LENGTH {
...
pub fn decrypt_sync_field(value: &str, key: &[u8; 32]) -> Result<String, String> {
    if !value.starts_with(SYNC_ENC_PREFIX) {
        return Ok(value.to_string());
    }
    let data = BASE64_STD
        .decode(&value[SYNC_ENC_PREFIX.len()..])
        .map_err(|_| "invalid base64".to_string())?;
    if data.len() < SYNC_IV_LENGTH + SYNC_TAG_LENGTH {
        return Err("ciphertext too short".to_string());
    }
...
pub fn encrypt_sync_data(data: &mut SyncData, key: &[u8; 32]) -> Result<(), String> {
    for conn in &mut data.connections {
        if let Some(pwd) = conn.password.as_ref() {
            conn.password = Some(encrypt_sync_field(pwd, key)?);
        }
        if let Some(pk) = conn.private_key.as_ref() {
            conn.private_key = Some(encrypt_sync_field(pk, key)?);
        }
        if let Some(pp) = conn.proxy_password.as_ref() {
            conn.proxy_password = Some(encrypt_sync_field(pp, key)?);
        }
    }
    for key_row in &mut data.ssh_keys {
        key_row.private_key = encrypt_sync_field(&key_row.private_key, key)?;
        if let Some(pp) = key_row.passphrase.as_ref() {
            key_row.passphrase = Some(encrypt_sync_field(pp, key)?);
        }
    }
    for preset in &mut data.presets {
        preset.password = encrypt_sync_field(&preset.password, key)?;
    }
    Ok(())
}
...
pub fn decrypt_sync_data(data: &mut SyncData, key: &[u8; 32]) -> Result<(), String> {
    for conn in &mut data.connections {
        if let Some(pwd) = conn.password.as_ref() {
            conn.password = Some(decrypt_sync_field(pwd, key)?);
        }
        if let Some(pk) = conn.private_key.as_ref() {
            conn.private_key = Some(decrypt_sync_field(pk, key)?);
        }
        if let Some(pp) = conn.proxy_password.as_ref() {
            conn.proxy_password = Some(decrypt_sync_field(pp, key)?);
        }
    }
    for key_row in &mut data.ssh_keys {
        key_row.private_key = decrypt_sync_field(&key_row.private_key, key)?;
        if let Some(pp) = key_row.passphrase.as_ref() {
            key_row.passphrase = Some(decrypt_sync_field(pp, key)?);
        }
    }
    for preset in &mut data.presets {
        preset.password = decrypt_sync_field(&preset.password, key)?;
    }
    Ok(())
}
