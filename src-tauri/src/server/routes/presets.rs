...
pub async fn get_presets(State(db): State<Db>) -> Result<Json<ApiResponse<Vec<PresetPublicRow>>>, (StatusCode, Json<ApiResponse<Value>>)> {
...
pub async fn get_preset(
    ...
    .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let Some(row) = row else { return Err(err(StatusCode::NOT_FOUND, "预设不存在")); };
    Ok(ok(row))
}
...
pub async fn get_preset_credential(
    ...
    .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let Some(row) = row else { return Err(err(StatusCode::NOT_FOUND, "预设不存在")); };
    let password = db.crypto.decrypt(&row.encrypted_password)
        .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(ok(json!({ "username": row.username, "password": password })))
}
...
pub async fn create_preset(
    ...
    if body.name.trim().is_empty() || body.username.trim().is_empty() || body.password.trim().is_empty() {
    ...
    let encrypted_password = db.crypto.encrypt(&body.password)
        .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    Ok((StatusCode::CREATED, ok(PresetPublicRow { id, name: body.name, username: body.username, remark, created_at: now.clone(), updated_at: now })))
}
...
pub async fn update_preset(
    ...
    .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let Some(existing) = existing else { return Err(err(StatusCode::NOT_FOUND, "预设不存在")); };
    ...
    let username = match obj.get("username") { Some(Value::String(s)) => s.clone(), _ => existing.username.clone() };
    ...
    let encrypted_password = match obj.get("password") {
        Some(Value::String(s)) if !s.trim().is_empty() => {
            db.crypto.encrypt(s).map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        }
    ...
    Ok(ok(PresetPublicRow { id, name, username, remark, created_at: existing.created_at, updated_at }))
}
...
pub async fn delete_preset(
    ...
    sqlx::query("UPDATE connections SET preset_id = NULL, auth_type = 'password' WHERE preset_id = ?")
        .bind(&id).execute(&db.pool).await
        .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(ok_empty())
}
