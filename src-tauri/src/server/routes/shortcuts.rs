...
pub async fn create_shortcut(
    ...
    sqlx::query("INSERT INTO shortcuts (id, name, command, remark, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)")
        .bind(&id).bind(&body.name).bind(&body.command).bind(&remark).bind(sort_order).bind(&now).bind(&now)
...
pub async fn update_shortcut(
    ...
    let existing = sqlx::query_as::<_, ShortcutRow>(
        "SELECT id, name, command, remark, sort_order, created_at, updated_at FROM shortcuts WHERE id = ?",
    )
    ...
    sqlx::query("UPDATE shortcuts SET name = ?, command = ?, remark = ?, sort_order = ?, updated_at = ? WHERE id = ?")
        .bind(&name).bind(&command).bind(&remark).bind(sort_order).bind(&updated_at).bind(&id)
        .execute(&db.pool).await
        .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
